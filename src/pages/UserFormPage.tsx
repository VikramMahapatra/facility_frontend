...
export default function UserFormPage() {
  ...
  const loadAll = async () => {
    await withLoader(async () => {
      ...
      const userSiteId =
        user && formMode !== "create" ? (user as any).site_id : undefined;
      const userBuildingId =
        user && formMode !== "create"
          ? (user as any).building_block_id
          : undefined;
      ...
      if (user && formMode !== "create" && (user as any).tenant_spaces) {
  ...
  const handleSave = async (userData: Partial<User>) => {
    ...
    let response;
    if (formMode === "create") {
      response = await userManagementApiService.addUser(userData);
    } else if (formMode === "edit" && user) {
      const updatedUser = {
        ...user,
        ...userData,
        updated_at: new Date().toISOString(),
      };
      response = await userManagementApiService.updateUser(updatedUser);
    }
  ...
  const onSubmitForm = async (data: UserFormValues) => {
    ...
    if (isTenant && data.tenant_spaces) {
      ...
      const validUserSpaces = data.tenant_spaces
        .filter((space: any) => space.site_id && space.space_id)
        .map((space: any) => ({
    ...
    // If editing and password is empty, exclude it from the data
    if (
      formMode === "edit" &&
      (!processedData.password || processedData.password.trim() === "")
    ) {
      const { password, ...dataWithoutPassword } = processedData;
      await handleSave(dataWithoutPassword as Partial<User>);
    } else {
      await handleSave(processedData as Partial<User>);
    }
  };
