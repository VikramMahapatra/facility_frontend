...
const Login = () => {
  ...
  const handleVerifyOtp = async (otpValue?: string) => {
    ...
    try {
      const response = await authApiService.verifyOtp(mobileNumber, otpToVerify);
      if (response?.success) {
        const authResponse = response.data;
        if (authResponse?.needs_registration) {
          navigate('/signup', {
                ...
                <>
                  <div className="space-y-3">
                    ...
                    <Button
                      ...
                      disabled={isLoading || isMobileLoading || !mobileNumber || mobileNumber.length < 10}
                      variant="outline"
                      size="lg"
                      className="w-full"
                    >
                      <Phone className="w-5 h-5 mr-2" />
                      {isMobileLoading ? "Sending OTP..." : "Continue with Mobile"}
                    </Button>
                  </div>
