...
export function UserForm({
  ...
  const [existingUserWarning, setExistingUserWarning] = useState<string | null>(
    null,
  );
  ...
  const checkGlobalUser = async (email?: string, phone?: string) => {
    ...
    try {
      const res = await userManagementApiService.checkUserGlobal({
        email,
        phone,
      });
                  ...
                  render={({ field }) => (
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        disabled={isReadOnly || isGlobalUser}
                        onBlur={() =>
                          checkGlobalUser(field.value, getValues("phone"))
                        }
                      />
                      {errors.email && (
                        <p className="text-sm text-red-500">
