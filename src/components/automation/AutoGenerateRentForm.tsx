...
export const AutoGenerateRentForm = ({
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<AutoGenerateRentFormValues>({
    defaultValues: {
      month:
  ...
  const onSubmit = async (data: AutoGenerateRentFormValues) => {
    setIsSubmitting(true);
    try {
      const date = `${data.year}-${data.month}-01`;

      const response = await withLoader(async () => {
        return await leaseChargeApiService.autoGenerateRent(date);
      });

      if (response?.success) {
        toast.success("Rent charges generated successfully");
        reset();
        onClose();
        if (onSuccess) {
          onSuccess();
